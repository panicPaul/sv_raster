...
def rasterize_voxels(
        ...
        vox_fn,
        octlevels: torch.Tensor | None = None,
    ):
    ...
    n_duplicates, geomBuffer = _C.rasterize_preprocess(
        ...
        octree_paths,
        torch.empty(0, dtype=torch.int8, device=device) if octlevels is None else octlevels,
        vox_centers,
        vox_lengths,
...
def mark_n_duplicates(
    ...
    n_duplicates, geomBuffer = _C.rasterize_preprocess(
        ...
        octree_paths,
        torch.empty(0, dtype=torch.int8, device=octree_paths.device),
        vox_centers,
        vox_lengths,
