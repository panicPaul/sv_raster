    ...
    float* __restrict__ max_w)
{
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
    {
        ...
        {
            int j = j_lst[jj];
            const int prim_id = collected_prim_id[j];
            ...
            float3 rgb_value = {0.f, 0.f, 0.f};
            float3 normal_value = {0.f, 0.f, 0.f};
            if (unresolved)
            {
                ...
                float3 normal_acc = {0.f, 0.f, 0.f};
                float3 rgb_acc = {0.f, 0.f, 0.f};
                for (uint32_t mid = member_start; mid < member_end; ++mid)
                {
                    const uint32_t leaf_id = unresolved_member_ids[mid];
                    const float3 member_center = vox_centers[leaf_id];
                    const float member_length = vox_lengths[leaf_id];
                    float geo_params[8];
                    for (int k = 0; k < 8; ++k)
                        geo_params[k] = geos[leaf_id * 8 + k];
                    const float z_i = dot(member_center - ro, cam_forward);
                    const float pix_area = pixel_footprint_area_at_depth(z_i, W, H, tan_fovx, tan_fovy);
                    const float tau_i = unresolved_tau_proxy(geo_params, member_length, pix_area);
                    tau_sum += tau_i;
                    depth_weighted += tau_i * z_i;
                    rgb_acc = rgb_acc + tau_i * rgbs[leaf_id];
                    ...
                    normal_acc = normal_acc + tau_i * safe_rnorm(lin_n) * lin_n;
                }
                vol_int = tau_sum;
                if (tau_sum > 0.f)
                {
                    rgb_value = (1.f / tau_sum) * rgb_acc;
                    z_proxy = depth_weighted / tau_sum;
                    normal_value = safe_rnorm(normal_acc) * normal_acc;
                }
            }
            else
            {
                ...
                const float lin_nz = (
                    ...
                    (geo_params[0b000] + geo_params[0b010] + geo_params[0b100] + geo_params[0b110]));
                const float3 lin_n = make_float3(lin_nx, lin_ny, lin_nz);
                normal_value = safe_rnorm(lin_n) * lin_n;
            }
            ...
            if (need_depth)
            {
                ...
                {
                    dval = alpha * (unresolved ? z_proxy : 0.5f * (a + b));
                }
                D = D + T * dval;
                ...
                if (T > 0.5f)
                {
                    D_med_vox_id = unresolved ? -1 : collected_ref_id[j];
                    if (unresolved)
                        D_med = z_proxy;
                    D_med_is_proxy = unresolved;
                    D_med_T = T;
                }
            }
            ...
            // Distortion depth
            if (need_distortion)
                Ddist = Ddist + pt_w * (unresolved ? depth_contracted(z_proxy) : 0.5f * (depth_contracted(a) + depth_contracted(b)));

            // Normal
            if (need_normal)
            {
                N = N + pt_w * normal_value;
            }
            ...
            if (track_max_w)
            {
                if (unresolved)
                {
                    for (uint32_t mid = collected_member_start[j]; mid < collected_member_end[j]; ++mid)
                    {
                        const uint32_t leaf_id = unresolved_member_ids[mid];
                        atomicMax(((int*)max_w) + leaf_id, *((int*)(&pt_w)));
                    }
                }
                else
                {
                    atomicMax(((int*)max_w) + collected_ref_id[j], *((int*)(&pt_w)));
                }
            }
        }
    ...
    if (need_depth && inside && !D_med_is_proxy && D_med_vox_id != -1)
    {
    ...
    dim3 grid)
{
    ...
    getBboxTileRectXYXY(bboxes[idx], tile_min, tile_max, grid);
    ...
    bool debug)
{
    ...
    cudaMemcpy(&primitive_count, geomState.primitive_count, sizeof(uint32_t), cudaMemcpyDeviceToHost);
    CHECK_CUDA(debug);
    ...
    cudaMemcpy(
        &num_rendered,
        geomState.n_duplicates_scan + primitive_count - 1,
        sizeof(int),
    ...
    {
        duplicateWithKeys<SortKey128> <<<(primitive_count + 255) / 256, 256>>> (
            primitive_count,
            geomState.primitive_octree_paths,
            geomState.bboxes,
    ...
    {
        duplicateWithKeys<uint64_t> <<<(primitive_count + 255) / 256, 256>>> (
            primitive_count,
            geomState.primitive_octree_paths,
            geomState.bboxes,
            geomState.cam_quadrant_bitsets,
