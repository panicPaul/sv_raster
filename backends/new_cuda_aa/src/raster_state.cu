...
{
    GeometryState geom;
    obtain(chunk, geom.primitive_count, 1, 128);
    obtain(chunk, geom.resolved_count, 1, 128);
    obtain(chunk, geom.unresolved_count, 1, 128);
    obtain(chunk, geom.proxy_count, 1, 128);
    obtain(chunk, geom.n_duplicates, P, 128);
    obtain(chunk, geom.n_duplicates_scan, P, 128);
    obtain(chunk, geom.leaf_bboxes, P, 128);
    obtain(chunk, geom.leaf_cam_quadrant_bitsets, P, 128);
    obtain(chunk, geom.bboxes, P, 128);
    obtain(chunk, geom.cam_quadrant_bitsets, P, 128);
    obtain(chunk, geom.primitive_octree_paths, P, 128);
    obtain(chunk, geom.primitive_ref_ids, P, 128);
    obtain(chunk, geom.primitive_is_proxy, P, 128);
    obtain(chunk, geom.primitive_member_starts, P, 128);
    obtain(chunk, geom.primitive_member_ends, P, 128);
    obtain(chunk, geom.unresolved_bucket_keys, P, 128);
    obtain(chunk, geom.unresolved_bucket_keys_sorted, P, 128);
    obtain(chunk, geom.unresolved_member_ids, P, 128);
    obtain(chunk, geom.unresolved_member_ids_sorted, P, 128);
    obtain(chunk, geom.unresolved_group_start_flags, P, 128);
    obtain(chunk, geom.unresolved_group_ids, P, 128);
    ...
    cub::DeviceRadixSort::SortPairs(
        nullptr, geom.sort_size,
        geom.unresolved_bucket_keys, geom.unresolved_bucket_keys_sorted,
        geom.unresolved_member_ids, geom.unresolved_member_ids_sorted, P);
    obtain(chunk, geom.sorting_temp_space, geom.sort_size, 128);
    return geom;
}
    ...
    const torch::Tensor& geomState)
{
    ...
    cudaMemcpy(new_geomState.primitive_count, &P, sizeof(uint32_t), cudaMemcpyHostToDevice);
    cudaMemcpy(new_geomState.resolved_count, &P, sizeof(uint32_t), cudaMemcpyHostToDevice);
    uint32_t zero = 0;
    cudaMemcpy(new_geomState.unresolved_count, &zero, sizeof(uint32_t), cudaMemcpyHostToDevice);
    cudaMemcpy(new_geomState.proxy_count, &zero, sizeof(uint32_t), cudaMemcpyHostToDevice);
